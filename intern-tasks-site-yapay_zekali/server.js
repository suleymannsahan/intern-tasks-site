...
// Kategori + döküman/bitiş tarihine göre 6 aşamalı iş planı üretir.
// Geçmiş gerçek sürelerden "akıllı ağırlık" hesaplar.
// ============================================================
...
// ===== AI İŞ PLANI ŞEMASI (güvenli, sadece ekleme) =====
// tasks tablosuna start_date + is_plani sütunları; asama_gecmisi (akıllı ağırlık için
// tarihsel süreler) ve asama_bildirimleri (gecikme/onay uyarıları) tabloları.
// NOT: Orijinal "notifications" tablosuna DOKUNULMAZ; AI bildirimleri ayrı tabloda tutulur.
async function initIsPlaniSchema() {
...
// (Kimlik istek gövdesinden okunur; projedeki diğer endpoint'lerle aynı mantık.)
// ============================================================
...
// ===== İŞ PLANI BİLDİRİMLERİ (izole tablo: asama_bildirimleri) =====
...
// ============================================================
// ===== GÖREV ASİSTANI (YAPAY ZEKA / CHATBOT) =====
// Kullanıcının görebildiği görevleri bağlam olarak Ollama'ya verir,
// serbest sohbetle Türkçe yanıtlar. Kimlik (userId/role/department)
// istek gövdesinden okunur — projedeki diğer endpoint'lerle aynı mantık.
// ============================================================
app.post('/api/chatbot', async (req, res) => {
  try {
    ...
    // Kullanıcının görebileceği görevleri çek — GET /api/tasks ile AYNI yetki mantığı:
    // ADMIN/HR tümünü, INTERN yalnızca kendine atananları, diğer roller kendi birimini görür.
...
// ============================================================
// ===== GENEL ASİSTAN (YAPAY ZEKA / serbest sohbet) =====
// /api/chatbot ile benzer; kullanıcının hem kendine atanan hem de
// kendi verdiği görevleri bağlam alır. Kimlik istek gövdesinden okunur.
// (Not: Bu endpoint için hazır bir arayüz yoktur; ileride bir sohbet
//  ekranından çağırmak istersen backend hazır dursun diye eklenmiştir.)
// ============================================================
app.post('/api/asistan', async (req, res) => {
  try {
    ...
    // Kullanıcının görebileceği görevler: kendine atanan + kendi verdiği
    let tasks = [];
    try {
      const r = await db.execute({
        sql: `SELECT tasks.*, users.name AS assignee_name
